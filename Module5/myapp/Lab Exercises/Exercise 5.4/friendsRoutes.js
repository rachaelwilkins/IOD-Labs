...
router.get('/filter', (req, res) => {
    ...
    let filterFirstLetter = req.query.letter.toUpperCase();
    ...
    if (filterFirstLetter) {
        ...
        let name = friend.name.toUpperCase();
        return name.startsWith(filterFirstLetter);
        });
    }    
...
// and an error response when there are no matches
        ...
        res.status(404).json({error: "No friends matching first letter "+filterFirstLetter})
    }  
})
...
// 3. Dynamic request param endpoint - get the friend matching the specific ID ie. /friends/3
    ...
    let friend = friends.find(user => user.id == userId)
})
...
// Modify this response with the matched friend, or a 404 if not found
    ...
    res.status(404).json({result:`Friend ${userId} not found`})
...
// Modify this response with the updated friend, or a 404 if not found
    if (updatedFriend) {
        ...
        res.status(404).json({ error: `Updated Friend with ID ${friendId} not found` });
        }
})
